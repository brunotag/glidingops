...
select tt.displayname
    ,SUM(CASE WHEN DATEDIFF(qgpdate,localdate) >= 0 AND  launchtype = 2 then 1 else 0 end) as Solo_Flights_Before_QGP_winch
	,SUM(CASE WHEN DATEDIFF(qgpdate,localdate) >= 0 AND launchtype = 1 then 1 else 0 end) as Solo_Flights_Before_QGP_aerotow
...
select tt.displayname,
	...
	,SUM(CASE WHEN DATEDIFF(qgpdate,localdate) >= 0 AND launchtype = 2 then 1 else 0 end) as P2_Flights_Before_QGP_winch
	,SUM(CASE WHEN DATEDIFF(qgpdate,localdate) >= 0 AND launchtype = 1 then 1 else 0 end) as P2_Flights_Before_QGP_aerotow
...
select * from p2_flights_before_qgp;

select tp2.displayname, tp2.first_flight_in_PW,
	tp2.P2_Flights_Before_QGP_aerotow + tp2.P2_Flights_Before_QGP_winch as P2_Flights_Before_QGP,
    tp1.Solo_Flights_Before_QGP_aerotow + tp1.Solo_Flights_Before_QGP_winch as Solo_Flights_Before_QGP,
    tp1.Solo_Flights_Before_QGP_winch + tp2.P2_Flights_Before_QGP_winch as Winch_Launches_Before_QGP, 
    tp1.Solo_Flights_Before_QGP_aerotow + tp2.P2_Flights_Before_QGP_aerotow as aerotows_Before_QGP
