...
try {
        ...
        foreach ($gliderlist as $gld) {
            ...
            if (strlen($ICAO) > 0) {
                ...
                */
                ...
                try {
                    //example: https://www.gliding.net.nz/api/v1/tracking/2022-11-28/C81B5D/pings
                    $gnzData = $myGNZ->getFlarmData($dayStr, $ICAO);
                    $data = $gnzData['data'];
                    foreach ($data as $r) {
                        //TODO: replace hardcoded org (1)
                        $DB->createTrack(1, $gld, $r['thetime'], 0, $r['lat'], $r['lng'], $r['alt'], 'FlarmGNZ');
                    }
                } catch (exception $e) {
    ...
    echo "Exception running getFlarmTask\n" . $e;
}
