...
echo "     cd /var/www/html/lrv && php artisan migrate --force"
echo ""
echo "3. CLEAR SESSIONS (force all users to re-login)"
echo "     rm -f /var/lib/php/sessions/*"
echo ""
echo "4. CONFIG FILES TO CREATE (gitignored)"
echo "     - /var/www/html/config/mail.php         (SMTP settings)"
echo "       See config/mail.php.sample for template"
echo "     - /var/www/html/config/site.php         (if needed)"
echo ""
echo "5. GOOGLE SERVICE ACCOUNTS"
echo "   - Member photos: download key from GCP project membersphotosupdate"
echo "     Place at /usr/local/.gdrive/<key-file>.json"
echo "     Add crotab entry:"
echo "       0 0 * * * gdrive -c /usr/local/.gdrive --service-account <key> download query \"'<folder-id>' in parents\" --force --path /var/www/html/img/members >> /var/log/downloadGopsMembersPhotos.log 2>&1"
echo ""
echo "   - gops-reporting: download key from GCP project wwgcdashboard"
echo "     Place at /var/local/gops-reporting/google_sheet_cred.json"
echo "     Also set up /var/local/gops-reporting/config.json (see config.live for template)"
echo ""
echo "6. DNS"
