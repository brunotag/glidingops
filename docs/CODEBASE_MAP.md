...
| APIs | todayxml.php | Data endpoints |
| Display | MasterDisplay.php | Visual output |

### Database Pattern

**Old Style (vulnerable):**
```php
$q = "SELECT * FROM users WHERE usercode='$myusername'";
```

**Newer Style (still not parameterized):**
```php
$sql = "SELECT * FROM " . TABLE_PREFIX . "members WHERE id = $id";
```

**Recommended (not widely used):**
```php
...
| Bootstrap Select | 1.12.1 | Enhanced dropdowns |
| flatpickr | (time-only) | Date/time picker |
| DataTables | (likely) | Table sorting/pagination |

**Loaded from:** `jsLibraies.php` - central include for all JS/CSS

### Cron Jobs

**SendTxt.php** - Likely run as cron job:
- Processes texts queue
- Converts to email (not SMS)
- Status 0 (pending) → 3 (sent via email)

**Check configuration:**
```bash
# Example cron (verify actual schedule)
*/5 * * * * php /path/to/SendTxt.php
```

---

## Largest/Complex Files
...
**Pattern recognition:** These 6 files represent 80% of the complexity. Understanding them unlocks the app.
...
### Billing
- towcharges.php, towcharges-list.php (pricing rules)
- billingoptions.php, billingoptions-list.php (flight billing options)
