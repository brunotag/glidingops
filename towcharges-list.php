...
require_once __DIR__ . '/helpers/permissions.php'; require_perm('tow-charges.manage'); ?>
<!DOCTYPE HTML>
<html>
...
{
 $rownum = $rownum + 1;
  echo "<tr class='";if (($rownum % 2) == 0)echo "even";else echo "odd";  echo "'>";if (true){$__e = (!isset($row[0]) || $row[0] === ''); echo "<td class='right' data-label='ID'" . ($__e ? " data-empty='1'" : "") . ">";echo "<a href='TowCharge?id=";echo $row[0];echo "'>";echo $row[0];echo "</a>";echo "</td>";}
if (true){echo "<td data-label='TOW PLANE'" . ((!isset($row[1]) || $row[1] === '') ? " data-empty='1'" : "") . ">";echo $row[1];echo "</td>";}
if (true){echo "<td class='right' data-label='TYPE (0/1)'" . ((!isset($row[2]) || $row[2] === '') ? " data-empty='1'" : "") . ">";echo $row[2];echo "</td>";}
if (true){echo "<td class='right' data-label='HEIGHT'" . ((!isset($row[3]) || $row[3] === '') ? " data-empty='1'" : "") . ">";echo $row[3];echo "</td>";}
if (true){echo "<td class='right' data-label='CLUB GLIDER'" . ((!isset($row[4]) || $row[4] === '') ? " data-empty='1'" : "") . ">";echo $row[4];echo "</td>";}
if (true){echo "<td data-label='MEMBER CLASS'" . ((!isset($row[5]) || $row[5] === '') ? " data-empty='1'" : "") . ">";echo $row[5];echo "</td>";}
if (true){$__e = (!isset($row[6]) || $row[6] == 0); echo "<td data-label='EFFECTIVE DATE'" . ($__e ? " data-empty='1'" : "") . ">";if ($row[6]!=0){$effective_from_d=new DateTime($row[6]); echo $effective_from_d->format('d/m/Y');}echo "</td>";}
if (true){echo "<td class='right' data-label='COST'" . ((!isset($row[7]) || $row[7] === '') ? " data-empty='1'" : "") . ">";echo $row[7];echo "</td>";}
  echo "</tr>";
}
?>
</table>
