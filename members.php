...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_SESSION['security'] & 16) {
        $medical_expire_f = InputChecker($_POST["medical_expire_i"]);
    }
    if ($_SESSION['security'] & 16) {
        $icr_expire_f = InputChecker($_POST["icr_expire_i"]);
    }
    if ($_SESSION['security'] & 16) {
        $bfr_expire_f = InputChecker($_POST["bfr_expire_i"]);
    }
    ...
    <form method="post" action="<?php echo htmlspecialchars('./Member'); ?>">
      <table>
        ...
        <?php if (true) {
            ...
            echo "<td>";
            echo "<input ";
            if (strlen($member_id_err) > 0) { echo "class='err' ";
            }
            echo "type='text' ";
            echo "name='member_id_i' ";
            echo "Value='";
            echo $member_id_f;
            echo "' ";
            echo " autofocus ";
        ...
        <?php if (true) {
            ...
            echo "></td>";
            echo "<td>";
            echo $enable_text_err;
            echo "</td></tr>";
        }
        ?>
        <?php if (true) {
            echo "<tr><td class='desc'>ENABLE EMALS</td><td></td>";
            echo "<td><input type='checkbox' name='enable_email_i[]' Value='1' ";
            if ($enable_email_f == 1) { echo "checked";
            }
            echo "></td>";
            echo "<td>";
            echo $enable_email_err;
            echo "</td></tr>";
        }
        ?>
          ...
          <td>
            ...
            $allRoles->each(
                function ($role) use ($userRoles) {
                    ...
                    $key = "role-{$role->id}";
                    ?>
              <input type='checkbox' value="<?php echo $role->id ?>" <?php echo ($selected) ? 'checked' : '' ?> name='roles[]' id='<?php echo $key ?>' />
              <label for='<?php echo $key ?>'><?php echo $role->name ?></label>
                    <?php
                }
            );
            ?>
      ...
      <table>
        <tr>
          <td>
            ...
            ?>
            <input type="submit" name='tran' value='<?php echo $submitValue ?>'>
          </td>
          <td>
