...
require_once __DIR__ . '/helpers/permissions.php'; require_perm('charges.manage'); ?>
<!DOCTYPE HTML>
<html>
...
{
 $rownum = $rownum + 1;
  echo "<tr class='";if (($rownum % 2) == 0)echo "even";else echo "odd";  echo "'>";if (true){$__e = (!isset($row[0]) || $row[0] === ''); echo "<td class='right' data-label='ID'" . ($__e ? " data-empty='1'" : "") . ">";echo "<a href='OtherCharge?id=";echo $row[0];echo "'>";echo $row[0];echo "</a>";echo "</td>";}
if (true){echo "<td data-label='CHARGE NAME'" . ((!isset($row[1]) || $row[1] === '') ? " data-empty='1'" : "") . ">";echo $row[1];echo "</td>";}
if (true){echo "<td data-label='LOCATION'" . ((!isset($row[2]) || $row[2] === '') ? " data-empty='1'" : "") . ">";echo $row[2];echo "</td>";}
if (true){echo "<td data-label='VALID FROM'" . ((!isset($row[3]) || $row[3] === '') ? " data-empty='1'" : "") . ">";echo $row[3];echo "</td>";}
if (true){echo "<td class='right' data-label='AMMOUNT'" . ((!isset($row[4]) || $row[4] === '') ? " data-empty='1'" : "") . ">";echo $row[4];echo "</td>";}
if (true){echo "<td class='right' data-label='CHARGE EVERY FLIGHT'" . ((!isset($row[5]) || $row[5] === '') ? " data-empty='1'" : "") . ">";echo $row[5];echo "</td>";}
if (true){echo "<td class='right' data-label='MAX ONCE PER DAY'" . ((!isset($row[6]) || $row[6] === '') ? " data-empty='1'" : "") . ">";echo $row[6];echo "</td>";}
if (true){echo "<td class='right' data-label='MONTHLY'" . ((!isset($row[7]) || $row[7] === '') ? " data-empty='1'" : "") . ">";echo $row[7];echo "</td>";}
if (true){echo "<td data-label='COMMENTS'" . ((!isset($row[8]) || $row[8] === '') ? " data-empty='1'" : "") . ">";echo $row[8];echo "</td>";}
  echo "</tr>";
}
?>
</table>
